...
void print_frame()
{
    const u_short MIN_WIDTH = 32;
    const u_short MIN_HEIGHT = 16;

    const u_short MAX_WIDTH = 200;
    const u_short MAX_HEIGHT = 100;
    ...
    for (u_short y = 2; y < height + 1; ++y) {
        index += sprintf(buffer + index, "\e[%d;1H│", y);
        index += sprintf(buffer + index, "\e[%d;%dH│", y, width);
    }

    // Title and command borders
    for (u_short x = 2; x < width; ++x) {
        index += sprintf(buffer + index, "\e[1;%dH_", x);
        index += sprintf(buffer + index, "\e[%d;%dH_", height - 1, x);
    }
...
void get_catalog(char *path)
{
    ...
    if(h != INVALID_HANDLE_VALUE){
        ...
        fprintf(stderr, "Error: failed to opening directory\n");
    }
}